...
from train_smoke_baseline import (
    BUNDLE_ROOT,
    DEFAULT_OUTPUT_ROOT,
    DEFAULT_SMOKE_DIR,
    DEPTH_MEAN,
...
DEFAULT_DATASET_ROOT = BUNDLE_ROOT.parents[1] / "datasets" / "v4" / "kitti_smoke_1280x384_lb"
DEFAULT_CONFIG_FILE = str(BUNDLE_ROOT / "configs" / "smoke_geometry_v2_b16_60ep.yaml")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the SMOKE geometry-v2 bundle.")
...
def main() -> None:
    ...
    output_dir = (
        ...
        else (DEFAULT_OUTPUT_ROOT / "smoke_geometry_v2" / f"seed_{args.seed}").resolve()
    )
    ...
    iters_per_epoch = -(-n_train // batch_size) if n_train is not None and batch_size > 0 else None
    ...
    print("[bundle-smoke-geometry-v2] cwd:", smoke_dir)
    print("[bundle-smoke-geometry-v2] cmd:", " ".join(cmd))
    print("[bundle-smoke-geometry-v2] dataset:", dataset_root)
    print("[bundle-smoke-geometry-v2] output:", output_dir)
