...
common_meta_keys = (
    ...
    'num_views', 'ego2global', 'axis_align_matrix', 'known_dims',
    'known_gravity_y')
...
model = dict(
    ...
    bbox_head=dict(
        ...
        pred_attrs=False,
        pred_velo=False,
        group_reg_dims=(1, 1, 1),  # offset_u, log_dv_delta, local yaw
        reg_branch=(
            (256, ),  # offset_u
            (256, ),  # log_dv_delta
            (256, )  # yaw
        ),
        geov3_base_dims=base_dims,
        geov3_base_y=base_gravity_center_y,
        geov3_depth_ref=base_depths[0][0],
        bbox_coder=dict(
            type='FCOS3DGeoV3BBoxCoder',
            code_size=3)),
    train_cfg=dict(
        allowed_border=0,
        pos_weight=-1,
...
train_pipeline = [
    ...
    dict(type='LoadFCOS3DGeoV3Meta', expected_dims=base_dims),
    dict(
