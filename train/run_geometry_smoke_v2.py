"""
Launch the restricted-DoF geometry-v2 model through official SMOKE training code.

`geometry_v2` keeps the reduced output parameterization used by `geometry`,
but switches the regression loss to an official-SMOKE-style decomposition:
orientation / dimension / location. Since dimensions are assumed known,
the dimension term is zero and location internally absorbs depth and
horizontal offset.
"""
...
DEFAULT_CONFIG_FILE = "configs/smoke_geometry_v2_b16_60ep.yaml"

TRUCK_L = 5.103683
TRUCK_H = 1.918924
TRUCK_W = 1.868429
DEPTH_MEAN = 5.331445990264416
DEPTH_STD = 2.6893772919824164
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Launch the official-SMOKE geometry-v2 fork through plain_train_net.py."
    )
    parser.add_argument(
        "--smoke-dir",
        type=Path,
    ...
    parser.add_argument(
        "--eval-only",
        action="store_true",
        help="Run geometry-v2 eval-only mode via official SMOKE test path.",
    )
    parser.add_argument(
        "--dataset-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory. If omitted, uses results/geometry_v2/seed_<seed>.",
    )
    ...
    opts = _set_opt(opts, "MODEL.SMOKE_HEAD.MODE", "geometry_v2")
    ...
    output_dir = (
        ...
        else (DEFAULT_OUTPUT_ROOT / "geometry_v2" / f"seed_{args.seed}").resolve()
    )
    ...
    print("[geometry-smoke-v2] cwd:", smoke_dir)
    print("[geometry-smoke-v2] cmd:", " ".join(cmd))
    print("[geometry-smoke-v2] dataset:", dataset_root)
    print("[geometry-smoke-v2] output:", output_dir)
    print("[geometry-smoke-v2] seed:", args.seed)
    if args.enable_mps_fallback or auto_mps_fallback:
        print("[geometry-smoke-v2] note: PYTORCH_ENABLE_MPS_FALLBACK=1 is enabled.")
